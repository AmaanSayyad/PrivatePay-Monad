// Dummy asset data for fallback/development — Monad Testnet only (chainId 10143)
import { MONAD_LOGO_ICON } from "../../../config.js";

const MONAD_CHAIN_ID = 10143;

export const ASSETS_DUMMY = {
  "aggregatedBalances": {
    "native": [
      {
        "chainId": MONAD_CHAIN_ID,
        "balance": 0.17,
        "nativeToken": "MON",
        "logo": MONAD_LOGO_ICON
      }
    ],
    "erc20": [
      {
        "chainId": MONAD_CHAIN_ID,
        "address": "0x53844F9577C2334e541Aec7Df7174ECe5dF1fCf0",
        "balance": 350,
        "logo": "https://cryptologos.cc/logos/usd-coin-usdc-logo.png",
        "name": "USDC Mock",
        "symbol": "USDC",
        "decimals": 18
      }
    ]
  },
  "stealthAddresses": [
    {
      "address": "0xe30e8F2A9135880952887A24CBd41cBE1f97b48A",
      "ephemeralPub": "0x0211ad17346ae99283e776432d0cfa611c49a26308b885ef061b920a8cc65a899e",
      "viewHint": "0xbb",
      "isTransacted": true,
      "createdAt": "2024-10-30T00:22:03.092Z",
      "nativeBalances": [
        {
          "chainId": MONAD_CHAIN_ID,
          "balance": 0.02,
          "nativeToken": "MON",
          "logo": MONAD_LOGO_ICON
        }
      ],
      "erc20Balances": []
    },
    {
      "address": "0x3641aa178303fCf35D5DBe4B207895Ec3A7E872e",
      "ephemeralPub": "0x02a9a41bc30b7c776df71b10578e7493db0106845b7b1764af0dc2d97fb34ac3a3",
      "viewHint": "0x91",
      "isTransacted": true,
      "createdAt": "2024-10-30T00:23:19.936Z",
      "nativeBalances": [
        {
          "chainId": MONAD_CHAIN_ID,
          "balance": 0.05,
          "nativeToken": "MON",
          "logo": MONAD_LOGO_ICON
        }
      ],
      "erc20Balances": [
        {
          "chainId": MONAD_CHAIN_ID,
          "address": "0x53844F9577C2334e541Aec7Df7174ECe5dF1fCf0",
          "balance": 150,
          "logo": "https://cryptologos.cc/logos/usd-coin-usdc-logo.png",
          "name": "USDC Mock",
          "symbol": "USDC",
          "decimals": 18
        }
      ]
    },
    {
      "address": "0xC0f6685AFACb376bcAF528a18ac13A44e3E4C6be",
      "ephemeralPub": "0x02474fb5037a4d82c86c83ffc5fd3b8f044cd7ffd2e715cddb1d1526440113a2bb",
      "viewHint": "0xba",
      "isTransacted": true,
      "createdAt": "2024-10-30T00:24:11.787Z",
      "nativeBalances": [
        {
          "chainId": MONAD_CHAIN_ID,
          "balance": 0.1,
          "nativeToken": "MON",
          "logo": MONAD_LOGO_ICON
        }
      ],
      "erc20Balances": [
        {
          "chainId": MONAD_CHAIN_ID,
          "address": "0x53844F9577C2334e541Aec7Df7174ECe5dF1fCf0",
          "balance": 50,
          "logo": "https://cryptologos.cc/logos/usd-coin-usdc-logo.png",
          "name": "USDC Mock",
          "symbol": "USDC",
          "decimals": 18
        }
      ]
    }
  ]
};
